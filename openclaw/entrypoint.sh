...
# Clone repo into workspace/repo/ using sparse checkout
# The bot gets app code (src/, trading/, etc.) but NOT deploy config
# (openclaw/, Dockerfile.fly, fly.toml) — those live on the persistent volume
# and are managed by the developer locally, not the bot.
set +e
REPO_DIR="$OC_DIR/workspace/repo"
REPO_URL="https://${GIT_REMOTE_TOKEN}@github.com/Readysoon/polymarket-mcp-server-fork.git"

if [ ! -d "$REPO_DIR/.git" ]; then
    echo "  Cloning repo (sparse) into workspace/repo/..."
    rm -rf "$REPO_DIR"
    if [ -n "$GIT_REMOTE_TOKEN" ]; then
        ...
        # Sparse checkout: exclude deploy infrastructure the bot shouldn't touch
        git sparse-checkout init --cone 2>&1
        git sparse-checkout set '/*' 2>&1
        # Exclude deploy-only paths (cone mode uses !path syntax in sparse-checkout file)
...
SPARSE
        ...
        echo "  Git: repo cloned (sparse — deploy files excluded)" || {
    ...
    # Existing clone: ensure sparse checkout rules are up to date
    if [ -d "$REPO_DIR/.git" ]; then
        cat > "$REPO_DIR/.git/info/sparse-checkout" << 'SPARSE'
/*
!openclaw/
!Dockerfile.fly
!fly.toml
